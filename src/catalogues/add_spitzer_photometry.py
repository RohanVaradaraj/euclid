...
if crossmatch:
    ...
    x_missing, y_missing = vista_cat['X_IMAGE'], vista_cat['Y_IMAGE']
...
with fits.open(image_dir / 'COSMOS_ch1_COMPLETE_microJy.fits') as hdu:
    ...
    #? Slightly larger aperture, https://irsa.ipac.caltech.edu/data/SPITZER/docs/irac/iracinstrumenthandbook/IRAC_Instrument_Handbook.pdf (pg 58, 3 pixels, larger bkg annulus)
    #flux, fluxerr, flag = sep.sum_circle(data, x_missing, y_missing, 1.8/0.15, subpix = 5)
    flux_count, fluxerr, flag = sep.sum_circle(data, x_missing, y_missing, 1.2/0.15, subpix = 5)
    flux_count = np.array(flux_count)
    flux = 10 ** (-(48.6+23.9)/2.5) * flux_count
    #flux = flux * 1.1132 
    flux = flux * 1.2026 
    print('flux extracted')
...
if crossmatch:
    ...
    vista_cat['err_ch1cds'] = error_ch1
    vista_cat['err_ch2cds'] = error_ch2

print('Added missing IRAC errors')

# Save the VISTA catalogue with the IRAC photometry
vista_file = vista_file.replace('.fits', '_with_irac_handbook.fits')
vista_cat.write(vista_dir /vista_file, format='fits', overwrite=True)
print('Saved VISTA catalogue with IRAC photometry:', vista_dir/vista_file)
