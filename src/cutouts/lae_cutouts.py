...
def Cutout(ra: float, dec:float, contained_in: Optional[np.array] = None, size: float = 10.0, 
    ...
    #### I ####
    with fits.open(vista_dir / 'HSC-Y_DR3.fits') as hdu_I:
    ...
    if cweb_tile != '0':
        angle = 0 - hdr_CWEB['PA_APER']
        cutout_f115w_cweb.data = rotate(cutout_f115w_cweb.data, angle, reshape=False)
        cutout_f150w_cweb.data = rotate(cutout_f150w_cweb.data, angle, reshape=False)
        cutout_f277w_cweb.data = rotate(cutout_f277w_cweb.data, angle, reshape=False)
        cutout_f444w_cweb.data = rotate(cutout_f444w_cweb.data, angle, reshape=False)
