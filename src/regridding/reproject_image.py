...
for field in fields:
    ...
    if instrument.lower() == 'jwst':
        ...
        for filter_name in filter_names:
            ...
            image_list = glob.glob(str(output_dir / 'CWEB' / '*' / f'CWEB-{filter_name}-*fits'))
            ...
            os.system(f'~/swarp/bin/swarp {images} -WEIGHT_IMAGE {errors} -c regrid.swarp -IMAGEOUT_NAME {str(save_image)} -WEIGHTOUT_NAME {str(save_error)} -WEIGHT_TYPE MAP_RMS {keywords}')
    ...
    if instrument.lower() == 'vista':
        ...
        for filter_name in filter_names:
            ...
            vista_image = vista_dir / f'UVISTA_p2m-{filter_name}_full_hr_v10.fits'
            vista_weight = vista_dir / f'UVISTA_p2m-{filter_name}_full_hr_v10_weight.fits'
