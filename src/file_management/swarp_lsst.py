...
Swarp together the LSST tiles to make a full mosaic, resampled to VISTA and on same plate scale.

Adapted from swarp_fornax.py: Friday 13th March 2026.
"""
...
#! Switches/variables
filter_names = ['u', 'g', 'r', 'i', 'z', 'y'] #? LSST filter(s) we want to resample/swarp
...
# Output directory
output_dir = lsst_dir / 'mosaics'

#! Load the Euclid tile names that lie in the VIDEO footprints.
lsst_in_video = pickle.load(open('lsst_DP1_within_video.pkl', 'rb')) #! DR1
print(lsst_in_video)
...
for video_tile in video_tiles:
    ...
    for filter_name in filter_names:
        ...
        fits_files = glob.glob(str(lsst_dir / filter_name / '*SCI.fits'))  
        rms_files = glob.glob(str(lsst_dir / filter_name / '*WHT.fits'))

        # Get the tile numbers from the file names
        all_tile_numbers = [filename.split('LSST_dp1_')[-1].split(f'_{filter_name}_SCI.fits')[0] for filename in fits_files]
        all_tile_numbers_rms = [filename.split('LSST_dp1_')[-1].split(f'_{filter_name}_WHT.fits')[0] for filename in rms_files]
        print(all_tile_numbers)
        
        #! Limit fits_files to those that lie in this specific VIDEO tile
        fits_files = [f for f in fits_files if f.split('LSST_dp1_')[-1].split(f'_{filter_name}_SCI.fits')[0] in lsst_in_video[video_tile]]
        rms_files = [f for f in rms_files if f.split('LSST_dp1_')[-1].split(f'_{filter_name}_WHT.fits')[0] in lsst_in_video[video_tile]]

        #! Also limit the tile numbers to those that lie in this specific VIDEO tile
        tile_numbers_here = [tile_number for tile_number in all_tile_numbers if tile_number in lsst_in_video[video_tile]]

        # Build a dictionary of {tile_id: filename} for each
        sci_dict = {filename.split('LSST_dp1_')[-1].split(f'_{filter_name}_SCI.fits')[0]: filename for filename in fits_files}
        rms_dict = {filename.split('LSST_dp1_')[-1].split(f'_{filter_name}_WHT.fits')[0]: filename for filename in rms_files}
        ...
        # Output file names
        output_file = output_dir / video_tile / f'{video_tile}_LSST_{filter_name}_MOSAIC.fits'
        outrms_file = output_dir / video_tile / f'{video_tile}_LSST_{filter_name}_MOSAIC_WHT.fits'
