...
def plot_footprints(footprints, close_polygons=True, color='blue', lw=2, alpha=0.8, tilename=False):
    for i, fp in enumerate(footprints):
        fp = fp.tolist()
        if close_polygons:
            fp.append(fp[0])  # Close the polygon
        ra, dec = zip(*fp)
        plt.plot(ra, dec, '-', color=color, alpha=alpha, lw=lw)
        if tilename:
...
plot_footprints(vista_footprints, color='black', lw=3, alpha=1)
...
#         fp.append(fp[0])  # Close polygon
#         ra, dec = zip(*fp)
#         plt.plot(ra, dec, '-', color=color, lw=2, alpha=0.9)
...
with open('euclid_within_video.pkl', 'wb') as f:
