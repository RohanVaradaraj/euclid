...
# Switch to filter hihg proper motion objects
remove_pm = True
...
for i, filter_name in enumerate(filter_names):
    ...
    if vista:
        ...
        stars_file = stars_dir / f'{filter_name}_vista_gaia_coords.ascii'
    ...
    if remove_pm:
        ...
        stars['pm'][stars['pm'] == '--'] = 0
        # convert to float
        stars['pm'] = stars['pm'].astype(float)

        stars = stars[stars['pm'] < 1]
    ...
    if vista:
        ...
        delta_ra = stars['RA_vista'] - stars['RA_Gaia']
        delta_dec = stars['DEC_vista'] - stars['DEC_Gaia']
    if jwst:
    ...
    # Find the subset of stars that lie outside the FWHM
    outside = np.where((np.sqrt(delta_ra**2 + delta_dec**2) > psf_fwhm[filter_names.index(filter_name)]/2))[0]

    # Outside the pixel scale and within fwhm
    outside_pix = np.where((np.sqrt(delta_ra**2 + delta_dec**2) > 0.15/2) & (np.sqrt(delta_ra**2 + delta_dec**2) < psf_fwhm[filter_names.index(filter_name)]/2))[0]
    ...
    plt.scatter(delta_ra[outside_pix], delta_dec[outside_pix], s=1, marker='x', color='blue', alpha=0.6)
    
    # # Save these to a file
    if gaia:
        outside_file = stars_dir / f'{filter_name}_outside_fwhm_euclid_gaia_coords.ascii'
        outside_pix_file = stars_dir / f'{filter_name}_outside_pixscale_euclid_gaia_coords.ascii'
    if vista:
        # outside_file = stars_dir / f'{filter_name}_outside_fwhm_vista_euclid_coords.ascii'
        # outside_pix_file = stars_dir / f'{filter_name}_outside_pixscale_vista_euclid_coords.ascii'
        outside_file = stars_dir / f'{filter_name}_outside_fwhm_vista_gaia_coords.ascii'
        outside_pix_file = stars_dir / f'{filter_name}_outside_pixscale_vista_gaia_coords.ascii'
    if jwst:
        outside_file = stars_dir / f'{filter_name}_outside_fwhm_jwst_euclid_coords.ascii'
        outside_pix_file = stars_dir / f'{filter_name}_outside_pixscale_jwst_euclid_coords.ascii'
    ...
    #     for i in outside:
    #         if gaia:
    #             f.write(f'{stars["RA_euclid"][i]} {stars["DEC_euclid"][i]} {stars["RA_Gaia"][i]} {stars["DEC_Gaia"][i]} {stars["pmra"][i]} {stars["pmdec"][i]} {stars["pm"][i]}\n')
    #         if vista:
    ...
    with open(outside_file, 'w') as f:
        if vista:
            f.write('# RA_Gaia DEC_Gaia RA_vista DEC_vista pmra pmdec pm\n')
        for i in outside:
            if vista:
                f.write(f'{stars["RA_Gaia"][i]} {stars["DEC_Gaia"][i]} {stars["RA_vista"][i]} {stars["DEC_vista"][i]} {stars["pmra"][i]} {stars["pmdec"][i]} {stars["pm"][i]}\n')
    with open(outside_pix_file, 'w') as f:
        if vista:
            f.write('# RA_Gaia DEC_Gaia RA_vista DEC_vista pmra pmdec pm\n')
        for i in outside:
            if vista:
    ...
    # Do the same thing for outside the pix
