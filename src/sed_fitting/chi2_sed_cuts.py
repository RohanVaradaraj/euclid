...
pristine_out_dir = lbg_dir + 'pristine' # Satisfies chi2 < 2sigma threshold, chi2_bd < 10, chi2_highz + 4 < chi2_lowz
...
two_sigma_thresh = dof + np.sqrt(2*dof)
...
# LBGs
good_lbg = lbg_table[:][lbg_table['col6'] < two_sigma_thresh] # High-z solution has chi2 < 2sigma threshold

# Brown dwarfs
good_bd = bd_table[:][bd_table['col21'] < 10]                 # BD solution has chi2 < 10

# Dusty galaxies 
condition_1 = dusty_table[:]['col2'] < 6                      # Best solution at z<6
condition_2 = dusty_table[:]['col6'] < dusty_table[:]['col15']   # Dusty solution has lower chi2 than LBG solution

good_dusty = dusty_table[:][condition_1 & condition_2]

# Lya emitters
condition_1 = lya_table[:]['col6'] < lbg_table[:]['col6']     # Lya solution has lower chi2 than LBG solution
condition_2 = lya_table['col2'] > 6                           # Lya solution at z>6

good_lya = lya_table[:][condition_1 & condition_2]
...
for category, (ids, out_dir) in categories.items():
    for ID in ids:
        file_name = f'Id{str(ID).zfill(9)}' 
        os.system(f'cp {file_name}.spec {output_dir / out_dir}')
