...
# Directory setup
zphot_folder = base_det + '_notBD'
zphot_dir = Path.cwd().parents[1] / 'data' / 'sed_fitting' / 'zphot' / field_name / 'best_fits' / zphot_folder

base_folder = base_det + 'notBD_z7Base'
base_dir = Path.cwd().parents[1] / 'data' / 'sed_fitting' / 'zphot' / field_name / 'best_fits' / base_folder


if not base_dir.exists():
    base_dir.mkdir(parents=True)

lbg_dir = zphot_dir.parent / (base_det + '_best_highz')
good_files = glob.glob(str(lbg_dir / '*.spec'))
...
if overwrite:
    for directory in [z7_dir, not_z7_dir]:
        for file in directory.glob('*.spec'):
            file.unlink()

# Populate the zphot dir with the good files from best_highz. Files from zphot_dir will then be copied over to the z7_dir and not_z7_dir
for file in good_files:
    if (zphot_dir / file).exists() and not (base_dir / file).exists():
        shutil.copy(lbg_dir / file, base_dir)

# Go through filesc in visual selection directory
spec_files = glob.glob(str(base_dir / '*.spec'))
...
for i, spec_file in enumerate(spec_files):
    ...
    original_sed_file = lbg_dir/ f'Id{ID.zfill(9)}.spec'
