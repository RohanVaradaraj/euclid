...
hsc_y['col2'] /= np.max(hsc_y['col2'])
vista_Y['col2'] /= np.max(vista_Y['col2'])
euclid_Y['col2'] /= np.max(euclid_Y['col2'])


# Plot
scale_val = 1.5e-30
hsc_y['col2'] *= scale_val
vista_Y['col2'] *= scale_val
euclid_Y['col2'] *= scale_val
...
with PdfPages(str(output_dir/output_pdf)) as pdf:
    ...
    for i, spec_file in enumerate(spec_files):
        ...
        print(object_flux['flux_J'])
        print(flux_to_mag(object_flux['flux_J']))
        ...
        for i in range(len(flux)):
            ...
            if sigma[i] < 2:
                ...
                ax1.errorbar(central_wavelengths[i], flux[i], yerr=error[i], fmt=marker, color=color, markersize=12, zorder=6, elinewidth=2, markeredgecolor='black')
